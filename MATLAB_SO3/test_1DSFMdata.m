...
%% Index of hardness of rotation averaging 
% See: Wilson et al. When is rotations averaging hard? ECCV 2016

D=diag(sum(A,2)); % Degree matrix
lambda2=max(abs(eigs(D-A,2,'sm'))); % second-smallest eigenvalue of the Laplacian matrix
...
[err_R] = error_R(R(:,:,i_cc),R_gt(:,:,cameras_gt(i_gt)+1));
...
time_IRLS=toc
[err_IRLS] = error_R(R_IRLS(:,:,i_cc),R_gt(:,:,cameras_gt(i_gt)+1));
...
% DMF setup
s=[3 15 30 3*ncams];% input size, hidden size 1, ..., output size
options.Wp=0.01;
options.Zp=0.01;
options.maxiter=100;
...
R_DMF=zeros(3,3,ncams);
[a,cam]=max(sum(A)); % node with highest degree is chosen
for k=1:ncams
...
[err_DMF] = error_R(R_DMF(:,:,i_cc),R_gt(:,:,cameras_gt(i_gt)+1));
...
figure,
hist(err_IRLS,20)
title('EIG+IRLS - absolute rotations')
xlabel('Error [degrees]')
set(gca,'FontSize',16,'LineWidth',3)
grid on
