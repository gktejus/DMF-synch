...
class MatrixCompletion(BaseProblem):
    ...
    def get_train_loss(self, e2e):
        ...
        return (self.ys.cuda() - self.ys_).pow(2).mean()

    def get_test_loss(self, e2e):
        return (self.w_gt - e2e.cuda()).reshape(-1).pow(2).mean()
...
def main(*, depth, hidden_sizes, n_iters, problem, train_thres, _seed, _log, _writer, _info, _fs):
    ...
    optimizer = lines[11].strip('\n').split("= ")[1].replace('"', '')
    initialization  = lines[12].strip('\n').split("= ")[1].replace('"','')
    lr = float(lines[13].strip('\n').split("= ")[1])
    config_dic = {"depth":depth , "init_scale": init_scale , "optimizer":optimizer , "initialization":initialization , "lr":lr, "dataset":dataset}
